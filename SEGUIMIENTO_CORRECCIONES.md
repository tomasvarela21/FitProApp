...
| Proxy de sesión | El frontend pasó a usar `/api` en todos los entornos. Vite reenvía esa ruta al backend local y Vercel la reenvía a Render antes del fallback de la SPA, para que la cookie HttpOnly sea de primera parte. Validación local: JSON válido, proxy con respuesta 401 esperada, 27/27 unitarias, build exitoso, lint con 0 errores y 11 advertencias conocidas, y 2/2 E2E en Chromium/WebKit. Pendiente de publicación y validación sobre ambos dominios. |
