...
| `SEC-001` | Fase 3 | Alta | El archivo local ignorado `backend/.env` contenía credenciales de base de datos activas en texto plano. | Resuelto: contraseña de PostgreSQL rotada en Supabase, conexiones locales sincronizadas y portapapeles limpiado. Supabase exige SSL y ambas URLs usan `sslmode=require`; Prisma y la API fueron verificados después del cambio | Cerrado |
...
| Proxy de sesión | El frontend pasó a usar `/api` en todos los entornos. Vite reenvía esa ruta al backend local y Vercel la reenvía a Render antes del fallback de la SPA, para que la cookie HttpOnly sea de primera parte. Los builds de producción fuerzan la ruta relativa aunque Vercel conserve una variable histórica con URL absoluta. Validación local: JSON válido, proxy con respuesta 401 esperada, 27/27 unitarias, build exitoso, lint con 0 errores y 11 advertencias conocidas, y 2/2 E2E en Chromium/WebKit. Pendiente de publicación y validación sobre ambos dominios. |
